...
//*************Ejercicio Clase: Baile de Triller		******//
//*************      Visual Studio 2017				    ******//
//************************************************************//
//************************************************************//
//Teclas Especiales para activar la animacion
//j/J =>manipula el eje X(+)
//g/G =>manipula el eje X(-)
//y/Y => manipula el eje Z(+)
//h/H => manipula el eje Z(-)
//p(+)/P(-) => girar
//b(+)/B(-) => Mover piernas
//r(+)/R(-) => Mover Brazos
//k => capturar frame
...
typedef struct _frame
{
	...
	float posX;//Variable para posicion en x
	float posY;//Variable para posicion en y
	float posZ;//Variable para posicion en z
	float incX;//Variable para  el incremento en x
	float incY;//Variable para  el incremento en y
	float incZ;//Variable para  el incremento en z
	float rotRodIzq;//Rotacion de la rodilla izquierda
	float rotRodDer;//Rotacion de la rodilla derecha
	float rotBraIzq;//Rotacion del brazo izquierdo
	float rotBraDer;//Rotacion del brazo derecho
	float rotInc;//Variable para el incremento de la rotacion
	float rotInc1;//Variable para el incremento1 de la rotacion
	float rotInc2;//Variable para el incremento2 de la rotacion
	float rotInc3;//Variable para el incremento3 de la rotacion
	float giroMonito;//Giro del monito
	float giroMonitoInc;//Variable para el incremento del giro del monito
...
void saveFrame(void)
{
	//Se guardan los movimientos del monito en su posicion en los ejes x,y &z
	printf("frameindex %d\n", FrameIndex);

	KeyFrame[FrameIndex].posX = posX;
	KeyFrame[FrameIndex].posY = posY;
	KeyFrame[FrameIndex].posZ = posZ;
	//Se guardan los movimientos de rotacion de piernas y brazos del monito
	KeyFrame[FrameIndex].rotRodIzq = rotRodIzq;
	KeyFrame[FrameIndex].rotRodDer = rotRodDer;
	KeyFrame[FrameIndex].rotBraIzq = rotBraIzq;
	KeyFrame[FrameIndex].rotBraDer = rotBraDer;
	//Se guarda el giro del monito
...
{
	//reset de las posiciones del monito en los ejes x,y &z
	posX = KeyFrame[0].posX;
	posY = KeyFrame[0].posY;
	posZ = KeyFrame[0].posZ;
	//reset de las rotaciones de los brazos y piernas del monito
	rotRodIzq = KeyFrame[0].rotRodIzq;
	rotRodDer = KeyFrame[0].rotRodDer;
	rotBraIzq = KeyFrame[0].rotBraIzq;
	rotBraDer = KeyFrame[0].rotBraDer;
	//Reset del giro del monito
...
{
	//KeyFrame para los incrementos en x,y &z
	KeyFrame[playIndex].incX = (KeyFrame[playIndex + 1].posX - KeyFrame[playIndex].posX) / i_max_steps;
	KeyFrame[playIndex].incY = (KeyFrame[playIndex + 1].posY - KeyFrame[playIndex].posY) / i_max_steps;
	KeyFrame[playIndex].incZ = (KeyFrame[playIndex + 1].posZ - KeyFrame[playIndex].posZ) / i_max_steps;
	//KeyFrame para los incrementos en las rotaciones
...
void monito()
{
	...
	glPushMatrix();//Pecho
	 ...
	 glPopMatrix();
	   ...
	   glPushMatrix();
	       ...
	       //Movimiento del brazo izquierdo (rotacion)
	...
	glPushMatrix();//Cintura
	  ...
	  glPushMatrix(); //Pie Izquierdo -->
	   glTranslatef(-0.75, -0.5, 0);
	   //Movimiento del pie izq
	   glRotatef(-20 + rotRodIzq, 1, 0, 0);
	   glTranslatef(0, -0.5, 0);
	   fig7.prisma(1.0, 0.5, 1, 0);
...
void InitGL(GLvoid)     // Inicializamos parametros
{
	...
	{
		KeyFrame[i].posX = 0;//KeyFrame de posicion en el eje x
		KeyFrame[i].posY = 0;//KeyFrame de posicion en el eje y
		KeyFrame[i].posZ = 0;//KeyFrame de posicion en el eje z

		KeyFrame[i].incX = 0;//KeyFrame del incremento de la posicion en el eje x
		KeyFrame[i].incY = 0;//KeyFrame del incremento de la posicion en el eje y
		KeyFrame[i].incZ = 0;//KeyFrame del incremento de la posicion en el eje z

		KeyFrame[i].rotRodIzq = 0;//KeyFrame de la rotacion de la rodilla izquierda
		KeyFrame[i].rotRodDer = 0;//KeyFrame de la rotacion de la rodilla derecha
		KeyFrame[i].rotBraDer = 0;//KeyFrame de la rotacion del brazo izquierdo
		KeyFrame[i].rotBraIzq = 0;//KeyFrame de la rotacion del brazo derecho

		KeyFrame[i].rotInc = 0;//KeyFrame del incremento de la rotacion
		KeyFrame[i].rotInc1 = 0;//KeyFrame del incremento de la rotacion1
		KeyFrame[i].rotInc2 = 0;//KeyFrame del incremento de la rotacion2
		KeyFrame[i].rotInc3 = 0;//KeyFrame del incremento de la rotacion3

		KeyFrame[i].giroMonito = 0;//KeyFrame del giro del monito
		KeyFrame[i].giroMonitoInc = 0;//KeyFrame del incremento del giro del monito
	}
	//NEW//////////////////NEW//////////////////NEW///////////
...
void keyboard(unsigned char key, int x, int y)  // Create Keyboard Function
{
	...
	case 'D':
		objCamera.Strafe_Camera(CAMERASPEED + 0.4);
		break;
//Casos para save & play del Frame
	case 'k':		
	case 'K':
	...
	case 'L':
		...
		break;
//Casos para hacer que el monito se mueva en las posiciones (+) y (-) de x & z
	case 'y':
	case 'Y':
		posZ++;
		printf("%f \n", posZ);
	...
	case 'J':
		...
		printf("%f \n", posX);
		break;
	//Rotacion de brazos al lado derecho
	case 'r':
		rotBraIzq++;
		rotBraDer++;
		printf("%f \n", rotBraIzq, rotBraDer);
		break;
   //Rotacion de los brazos al lado izquierdo
	case 'R':	
		rotBraIzq--;
		rotBraDer--;
		printf("%f \n", rotBraIzq, rotBraDer);
		break;
   //Rotacion de las rodillas al lado derecho  
	case 'b':
		rotRodIzq++;
		rotRodDer++;
		printf("%f \n", rotRodIzq, rotRodDer);
	...
	case 'B':
		...
		printf("%f \n", rotRodIzq, rotRodDer);
		break;
	//Giro del monito al lado derecho
	case 'p':
		giroMonito++;
		break;
	//Giro del monito al lado izquierdo
	case 'P':
		giroMonito--;
		break;
...
int main(int argc, char** argv)   // Main Function
{
	...
	glutCreateWindow("Practica 11: <<Baile Triller by: Shanik Trejo>>"); // Nombre de la Ventana
